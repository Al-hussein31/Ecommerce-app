...
const slider = function() {
    ...
    const activateDot = function(slide) {
        ...
        const dotsSlide = document.querySelector(`.dots__dot-image[data-slide="${slide}"]`);
...
const decreasingNumbers = function(num) {
    if (num.textContent > 1) num.textContent--;
};
const updateNumber = function(factor) {
    promoNumber.forEach((promoNumber)=>{
        ...
        if (!isNaN(currentNumber)) {
            ...
            if (newNumber && newNumber > 0) promoNumber.textContent = "$" + newNumber;
        }
    });
};
...
btnThumbnails.forEach((btn)=>{
    btn.addEventListener("click", function() {
        ...
        if (largeImage) // Changing the image
...
document.addEventListener("click", function(e) {
    if (!bookmarksMain.contains(e.target) && e.target !== navBtn) bookmarksMain.classList.add("hidden");
});
layouts.addEventListener("click", function(e) {
    e.preventDefault();
    const btn = e.target;
    if (btn.classList.contains("layout__images-img__fig")) openModal(e);
...
document.addEventListener("keydown", function(e) {
    if (e.key === "Escape" && !modelWindow.classList.contains("hidden")) closeModal();
});
...
},{}]},["aD7Zm","aenu9"], "aenu9", "parcelRequire52a8")
